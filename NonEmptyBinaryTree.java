...
public class NonEmptyBinaryTree<T extends Comparable<T>> extends BinaryTree<T> {
    ...
    public BinaryTree<T> delete(T d) {
        ...
        if (!this.find(d)) {
            return this;
        } else if (this.data.equals(d) && this.left.isEmpty() && this.left.isEmpty()) {
            return new EmptyBinaryTree<T>();
        } else if (this.data.equals(d)) {
            BinaryTree oldleft = this.left;
            BinaryTree oldright = this.right;
            BinaryTree<T> newroot = new EmptyBinaryTree<>();
            if (!oldleft.isEmpty()) {
                newroot = insert(newroot, oldleft);
            }
            if (!oldright.isEmpty()) {
                newroot = insert(newroot, oldright);
            }
            return newroot;
        } else {
            if (d.compareTo(data) < 0) {
    ...
    BinaryTree<T> insert(BinaryTree<T> root, BinaryTree<T> tree) {
        if (tree.isEmpty())
            return root;
        if (root.isEmpty()) {
            root = new NonEmptyBinaryTree<T>(((NonEmptyBinaryTree<T>) tree).data);
            insert(root, ((NonEmptyBinaryTree<T>) tree).left);
            insert(root, ((NonEmptyBinaryTree<T>) tree).right);
        } else {
            root.insert(((NonEmptyBinaryTree<T>) tree).data);
            insert(root, ((NonEmptyBinaryTree<T>) tree).left);
            insert(root, ((NonEmptyBinaryTree<T>) tree).right);
        }
        return root;
